...
function queryBabe(api) {
  return api.derive.session.info().pipe((0, _operators.switchMap)(info => {
    var _api$query$staking;

    return (0, _xRxjs.combineLatest)([(0, _xRxjs.of)(info), // we may have no staking, but have babe (permissioned)
  ...
  }), (0, _operators.map)(([info, [currentSlot, epochIndex, genesisSlot, optStartIndex]]) => [info, [currentSlot, epochIndex, genesisSlot, optStartIndex && optStartIndex.isSome ? optStartIndex.unwrap() : api.registry.createType('SessionIndex', 1)]]));
}
/**
...
function progress(instanceId, api) {
  return (0, _index.memo)(instanceId, () => api.query.babe ? queryBabe(api).pipe((0, _operators.map)(([info, slots]) => createDerive(api, info, slots))) : queryAura(api));
}