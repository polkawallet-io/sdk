...
var _index18 = require("./type/index.cjs");
...
function injectFunctions(instanceId, api, allSections) {
  ...
  return Object.keys(allSections).filter(sectionName => !deriveAvail[sectionName] || deriveAvail[sectionName].instances.some(q => queryKeys.includes(q)) || deriveAvail[sectionName].withDetect && deriveAvail[sectionName].instances.some(q => (api.registry.getModuleInstances(specName, q) || []).some(q => queryKeys.includes(q)))).reduce((deriveAcc, sectionName) => {
