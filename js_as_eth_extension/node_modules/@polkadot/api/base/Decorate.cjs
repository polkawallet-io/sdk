...
class Decorate extends _Events.Events {
  ...
  constructor(options, type, decorateMethod) {
    ...
    this.__phantom = new _util2.BN(0);
    this._consts = {};
    this._derive = void 0;
    this._errors = {};
    this._events = {};
  ...
  _injectDecorated(registry, fromEmpty, blockHash) {
    const decoratedMeta = (0, _metadata.expandMetadata)(registry.registry, registry.metadata); // clear the decoration, we are redoing it here
  ...
  _decorateStorageEntryAt(creator, decorateMethod, blockHash) {
    ...
    if (creator.iterKey && (creator.meta.type.isMap || creator.meta.type.isDoubleMap)) {
      decorated.entries = decorateMethod((0, _util.memo)((0, _classPrivateFieldLooseBase2.default)(this, _instanceId)[_instanceId], (...args) => this._retrieveMapEntries(creator, blockHash, args)));
      decorated.keys = decorateMethod((0, _util.memo)((0, _classPrivateFieldLooseBase2.default)(this, _instanceId)[_instanceId], (...args) => this._retrieveMapKeys(creator, blockHash, args)));
    }
    /* eslint-enable @typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-assignment */
  ...
  _retrieveMulti(keys) {
    ...
    return (0, _xRxjs.combineLatest)((0, _util2.arrayChunk)(keys, PAGE_SIZE_V).map(keys => queryCall(keys))).pipe((0, _operators.map)(_util2.arrayFlatten));
  }
  ...
  _retrieveMapKeys({
    ...
    return startSubject.pipe((0, _operators.switchMap)(queryCall), (0, _operators.map)(keys => keys.map(key => key.setMeta(meta, section, method))), (0, _operators.tap)(keys => {
      setTimeout(() => {
    ...
    (0, _operators.map)(_util2.arrayFlatten));
  }
  ...
  _retrieveMapEntries(entry, at, args) {
    ...
    return this._retrieveMapKeys(entry, at, args).pipe((0, _operators.switchMap)(keys => keys.length ? (0, _xRxjs.combineLatest)((0, _util2.arrayChunk)(keys, PAGE_SIZE_V).map(query)).pipe((0, _operators.map)(valsArr => (0, _util2.arrayFlatten)(valsArr).map((value, index) => [keys[index], value]))) : (0, _xRxjs.of)([])));
  }
