...
class Init extends _Decorate.Decorate {
  ...
  async getBlockRegistry(blockHash) {
    ...
    const registry = new _create.TypeRegistry(blockHash);
    const metadata = new _metadata.Metadata(registry, await this._rpcCore.state.getMetadata.raw(header.parentHash).toPromise());
