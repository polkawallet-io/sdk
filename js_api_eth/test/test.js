((window) => {
  ...
  async function runKeyringTestETH() {
    ...
    const signed = await eth.keyring.signMessage(message, acc.keystore, password);
    ...
    const passCheck1 = await eth.keyring.checkPassword(acc.keystore, password);
    expect(passCheck1.success, true);
    const passCheck2 = await eth.keyring.checkPassword(acc.keystore, password + "xx");
    expect(passCheck2.success, false);
    expect(passCheck2.error, "invalid password");

    console.log("change password");
    const passNew = "c111111";
    const changed = await eth.keyring.changePassword(acc.keystore, password, passNew);
    expect(changed.pubKey, acc.pubKey);
    expect(changed.address, acc.address);
    const passCheck3 = await eth.keyring.checkPassword(changed.keystore, password);
    expect(passCheck3.success, false);
    expect(passCheck3.error, "invalid password");
    const passCheck4 = await eth.keyring.checkPassword(changed.keystore, passNew);
